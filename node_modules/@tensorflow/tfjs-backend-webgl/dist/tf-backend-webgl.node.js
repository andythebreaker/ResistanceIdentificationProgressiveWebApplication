...
function compileProgram(gpgpu, program, inputs, output) {
    ...
    if (!tf.env().get('ENGINE_COMPILE_ONLY')) {
        return Object.assign({ program: program,
            ...
            outShapeInfo: outShapeInfo }, getUniformLocations(gpgpu, program, webGLProgram));
    }
    else {
        return {
            program: program,
...
var PackProgram = /** @class */ (function () {
    ...
    PackProgram.prototype.getSetup = function (dims) {
        ...
        var col = this.enableShapeUniforms ? "outShape[" + this.rank + " - 1]" :
            this.outputShape[this.rank - 1];
        var row = this.enableShapeUniforms ? "outShape[" + this.rank + " - 2]" :
...
var MathBackendWebGL = /** @class */ (function (_super) {
                ...
                switch (_c.label) {
                    case 0:
                        ...
                        if (dtype !== 'complex64' && tf.env().get('WEBGL_BUFFER_SUPPORTED')) {
                            ...
                            buffer = (_b = this.gpgpu).createBufferFromTexture.apply(_b, __spread([tmpData.texture.texture], getDenseTexShape(shape)));
                        }
    ...
    MathBackendWebGL.prototype.getValuesFromTexture = function (dataId) {
        ...
        if (tf.env().getBool('WEBGL_DOWNLOAD_FLOAT_ENABLED')) {
            ...
            var vals_1 = (_b = this.gpgpu).downloadMatrixFromPackedTexture.apply(_b, __spread([tmpData_1.texture.texture], getDenseTexShape(shape))).subarray(0, size);
    ...
    MathBackendWebGL.prototype.getUniformLocations = function () {
        ...
        try {
            for (var _c = __values(Object.entries(this.binaryCache)), _d = _c.next(); !_d.done; _d = _c.next()) {
                var _e = __read(_d.value, 2), binary = _e[1];
                var _f = getUniformLocations(this.gpgpu, binary.program, binary.webGLProgram), uniformLocations = _f.uniformLocations, customUniformLocations = _f.customUniformLocations, infLoc = _f.infLoc, nanLoc = _f.nanLoc, inShapesLocations = _f.inShapesLocations, inTexShapesLocations = _f.inTexShapesLocations, outShapeLocation = _f.outShapeLocation, outShapeStridesLocation = _f.outShapeStridesLocation, outTexShapeLocation = _f.outTexShapeLocation;
...
function fitPreluActivationWeightsIntoNhwcFormat(alpha, outputShape, isChannelsLast, backend) {
    ...
    // tensor.
    var alphaShape = alpha.shape;
    tf.util.assert(alphaShape.length <= 1 || alphaShape.length === 3, function () { return "WebGL conv2d only supports scalar, 1-D Tensor or 3-D " +
        "Tensor PReLU activation weights but got a tensor of " +
        ("rank-" + alphaShape.length + "."); });
    if (alphaShape.length === 1) {
        var outputChannels_1 = isChannelsLast ? outputShape[3] : outputShape[1];
        tf.util.assert(alphaShape[0] === 1 || alphaShape[0] === outputChannels_1, function () { return "WebGL conv2d PReLU activation weights (" + alphaShape + ") is " +
            "not compatible with the number of output channels " +
            ("(" + outputChannels_1 + ")."); });
    }
    else if (alphaShape.length === 3) {
        try {
            tf.broadcast_util.assertAndGetBroadcastShape(alphaShape, outputShape);
        }
        catch (e) {
            var errMsg = "WebGL conv2d PReLU activation weights (" + alphaShape + ") " +
        ...
        if (!isChannelsLast) {
            ...
            return transpose({ inputs: { x: alpha }, backend: backend, attrs: { perm: [1, 2, 0] } });
        }
    }
    return alpha;
}
...
function conv2dByMatMul(_a) {
    ...
    if (preluActivationWeights != null) {
        var preluActivationWeightsInNhwcFormat = fitPreluActivationWeightsIntoNhwcFormat(preluActivationWeights, convInfo.outShape, isChannelsLast, backend);
    ...
    if (canOptimize) {
        ...
        var xInNhwcFormat = isChannelsLast ?
            x :
            transpose({ inputs: { x: x }, backend: backend, attrs: { perm: [0, 2, 3, 1] } });
        var xInNhwcFormatShape = xInNhwcFormat.shape;
        var targetShape = xInNhwcFormatShape[0] * xInNhwcFormatShape[1] * xInNhwcFormatShape[2];
        ...
        var outInNHWCFormatShape = [
            convInfo.batchSize, convInfo.outHeight, convInfo.outWidth,
            convInfo.outChannels
        ];
        var outInNHWCFormat = reshape({ inputs: { x: result }, backend: backend, attrs: { shape: outInNHWCFormatShape } });
...
function conv2dWithIm2Row(_a) {
    ...
    var outInNHWCFormatShape = [1, outHeight, outWidth, convInfo.outChannels];
    var outInNHWCFormat = reshape({ inputs: { x: product }, backend: backend, attrs: { shape: outInNHWCFormatShape } });
    // If the data format is NCHW, then convert the output to be NCHW format.
    var out = isChannelsLast ?
        outInNHWCFormat :
        transpose({ inputs: { x: outInNHWCFormat }, backend: backend, attrs: { perm: [0, 3, 1, 2] } });
    if (!isChannelsLast) {
        intermediates.push(outInNHWCFormat);
    }
    intermediates.push(product);
...
var CumProgram = /** @class */ (function () {
    function CumProgram(op, shape, exclusive, reverse) {
        ...
        var val = exclusive ? initVal : "getX(" + getCoords(rank, 'coords', this.op) + ")";
...
function gatherV2(args) {
    ...
    if (tf.env().get('DEBUG')) {
        ...
        var _loop_1 = function (i) {
            ...
            tf.util.assert(index <= axisDim_1 - 1 && index >= 0, function () { return "GatherV2: the index value " + index + " is not in [0, " + (axisDim_1 - 1) + "]"; });
        };
...
var SearchSortedProgram = /** @class */ (function () {
    function SearchSortedProgram(batchSize, numInputs, numValues, side) {
        ...
        // iterations.
        var webGL1LoopHead = "for (int i = 0; i < " + Math.ceil(Math.log2(numInputs + 1)) + "; ++i) { if (left >= right) break;";
